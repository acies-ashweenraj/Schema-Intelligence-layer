    ...
    def _load_semantic_layer_tx(tx: Transaction, semantic_layer: Dict, client_id: str) -> None:
        ...
        for table_name, table_data in tables_dict.items():
            tx.run(
                ...
                is_pii=False, # `is_pii` is not explicitly provided in the semantic layer JSON, defaulting to False
            )
        ...
        for column in all_columns:
            ...
            tx.run(
                ...
                risk_category="LOW", # `risk_category` for columns is not explicitly provided in the semantic layer JSON, defaulting to LOW
        ...
        for rel_data in all_relationships_data:
            if rel_data.get("type") == "explicit":
                tx.run(
                    ...
                    cardinality="1:N", # `cardinality` is not explicitly provided in the semantic layer JSON, defaulting to "1:N"
                )
