    ...
    <div className="h-screen w-full bg-slate-50 grid grid-cols-1 lg:grid-cols-[340px_1fr]">
      ...
      <aside className="bg-white border-r border-slate-200 p-4 flex flex-col gap-4">
        ...
        <div className="border-t border-slate-200 pt-4 space-y-3">
          <p className="text-xs font-semibold text-slate-500 px-2 uppercase">Settings</p>
          <SettingSelect
            label="Client ID"
            value={chatSettings.client_id}
            onChange={(e) => setChatSettings((p) => ({ ...p, client_id: e.target.value }))}
      ...
      <main className="flex flex-col h-screen bg-slate-100">
        <div className="p-4 border-b bg-white/70 backdrop-blur-lg">
          <h3 className="text-lg font-bold text-slate-800">{activeChat?.title || "Chat"}</h3>
...
function Message({ message }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {message.error && (
        <div className="max-w-[80%] rounded-2xl bg-red-50 border border-red-200 text-red-800 p-3 text-sm font-medium">
          <p>
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
      <div className="p-3 border-b border-slate-200 flex justify-between items-center">
        <p className="text-xs font-bold text-slate-600">Query Result ({data.length} rows)</p>
        <Button onClick={downloadCSV} variant="soft" className="px-3 py-1.5 text-xs">
          Download as CSV
        </Button>
      </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {data.map((row, i) => (
              <tr key={i} className="hover:bg-slate-50">
                {headers.map((h) => (
                  <td key={h} className="px-4 py-2 text-slate-700 truncate max-w-xs">
                    {String(row[h])}
                  </td>
                ))}
              </tr>
